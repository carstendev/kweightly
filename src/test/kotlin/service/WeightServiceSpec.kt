...
class WeightServiceSpec : StringSpec({
    ...
    val weightService = WeightService(WeightRepository(db), Filter.NoOp) // authZ/authN disabled
    ...
    "/api/weights should correctly handle GET, POST, PUT and DELETE requests" {
        ...
        val responseAfterDelete = weightService(Request(Method.DELETE, "/api/weights/1").headers(requestClaimsHeader))
    ...
    "/api/weights should respect authN and not leak user information" {
        ...
        val verifyResourceNotDeleted = weightService(Request(Method.GET, "/api/weights").headers(requestClaimsHeaderOne))
